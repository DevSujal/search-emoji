  ...
  if (loader) {
    return (
      <div className="absolute transform left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
        <h1 className=" text-5xl text-white text-bold">Loading...</h1>
      </div>
    );
  } else {
    return (
        ...
        {emojis
          .filter(
            (emoji) =>
              ...
              emoji.name !== "helmet with white cross"
          )
          .map((emoji) => (
            <div
              key={emoji.id}
            ...
            >
              <h2 className="absolute  text-gray-200 font-semibold hidden z-10">
                {emoji.name}
              </h2>
              <h2
                className="text-7xl hover:cursor-pointer sm:text-8xl"
                onMouseOver={hover}
